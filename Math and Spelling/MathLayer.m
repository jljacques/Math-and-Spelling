...
//  Math and Spelling
//
//  Created by Jeremy Jacques on 2013-05-22.
//  Copyright Appstract Art 2013. All rights reserved.
//

#define kShapesMode 1
#define kNumberMode 2
#define kColourMode 3
#define kAnimalMode 4
...
-(void)valueInit {
 ...
 //   [self addChild:backgroundSprite z:0];
    ...
    outlineLoc0 = CGPointMake(206.8f, 77.4f);
    outlineLoc1 = CGPointMake(344.4f, 77.4f);
    outlineLoc2 = CGPointMake(206.8f, 214.6f);
    outlineLoc3 = CGPointMake(344.4f, 214.6f);
    
    playAgainLoc = CGPointMake(120.0f, 576.0f);
    playNextLoc = CGPointMake(120.0f, 384.0f);
    playPrevLoc = CGPointMake(120.0f, 192.0f);
    
    colourButtonLoc = CGPointMake(400.0f, 700.0f);
    shapeButtonLoc = CGPointMake(550.0f, 700.0f);
    numberButtonLoc = CGPointMake(700.0f, 700.0f);
    animalButtonLoc = CGPointMake(850.0f, 700.0f);
    
    buttonShrink = 0.45f;
    
    
    shapeArray = [[NSMutableArray arrayWithObjects:@"100",@"101",@"102",@"103",@"104",@"105",@"106",@"107",@"108",@"109", nil]retain];
    coloursArray = [[NSMutableArray arrayWithObjects:@"100",@"101",@"102",@"103",@"104",@"105",@"106",@"107", nil]retain];
    dragArray = [[NSMutableArray arrayWithObjects:@"100",@"101",@"102",@"103",@"104",@"105",@"106",@"107",@"108",@"109", nil]retain];
    dragNumArray = [[NSMutableArray arrayWithObjects:@"100",@"101",@"102",@"103",@"104",@"105",@"106",@"107",@"108",@"109", nil]retain];
    ...
    Black = ccc3(0, 0, 0);
    Brown = ccc3(155, 113, 70);
    Grey = ccc3(166, 168, 171);
...
-(void)dragLocator {
    if (level == 1) {
        ...
        dragLoc0 = CGPointMake(54.8f, 281.7f);
        dragLoc1 = CGPointMake(54.8f, 204.3f);
        dragLoc2 = CGPointMake(54.8f, 126.9f);
        dragLoc3 = CGPointMake(54.8f, 49.5f);
    }
    if (level == 2) {
...
-(void)getLabel:(NSArray*)label
      andColour:(NSArray*)colour {
    if (objectTouched == 0) {
        textInt = [[shapeArray objectAtIndex:0] integerValue];
        labelString = [NSMutableString stringWithString:[label objectAtIndex:textInt]];
        if (gameMode == kAnimalMode) {
            colourString = [NSMutableString stringWithString:[colour objectAtIndex:textInt]];
        }
        else{
...
-(void)getColour {
    ...
    else if ([colourString isEqualToString:@"Brown"]) {
        objectColour = Brown;
    }
    else if ([colourString isEqualToString:@"Grey"]) {
        objectColour = Grey;
    }
    else {
        objectColour = Black;
...
-(void)getObjectString:(NSArray*)objectArray {
    if (gameMode == kAnimalMode) {
        objectString = [NSMutableString stringWithFormat:@"%@.png",[objectArray objectAtIndex:shapeNum]];
    }
    else {
...
-(void)textSetup {
    ...
    if (gameMode == kShapesMode) {
        [self getLabel:shapeNames andColour:nil];
    }
    else if (gameMode == kNumberMode) {
        [self getLabel:numbers andColour:nil];
    }
    else if (gameMode == kColourMode) {
        [self getLabel:shapeNumberCombine andColour:nil];
    }
    else if (gameMode == kAnimalMode) {
        [self getLabel:animals andColour:animalColours];
    }
...
-(void)categorySetup {
    ...
    animals = [[NSArray arrayWithObjects:@"Aligator",@"Bear",@"Beaver",@"Bunny",@"Camel",@"Cat",@"Cow",@"Dog",@"Donkey",@"Elephant",@"Fox",@"Giraffe",@"Hippo",@"Lamb",@"Lion",@"Monkey",@"Moose",@"Mouse",@"Penguin",@"Pig",@"Raccoon",@"Rhino",@"Turtle", nil] retain];
    animalColours = [[NSArray arrayWithObjects:@"Green",@"Brown",@"Brown",@"Purple",@"Brown",@"Orange",@"Pink",@"Orange",@"Grey",@"Grey",@"Red",@"Orange",@"Brown",@"White",@"Orange",@"Brown",@"Brown",@"Brown",@"White",@"Pink",@"Brown",@"Grey",@"Brown", nil] retain];
...
-(void)endGame {
    ...
    animalLevel = [CCSprite spriteWithFile:@"Cat.png"];
    animalLevel.position = animalButtonLoc;
    animalLevel.scale = buttonShrink;
    [self addChild:animalLevel z:70];
     */
...
+(CCScene *) scene
{
	...
	MathLayer *layer = [MathLayer node];
...
{
    CCLOG(@"Test1");
    dragSprite0 = [CCSprite spriteWithFile:@"Eight_Blue.png"];
    ...
    dragSprite1 = [CCSprite spriteWithFile:@"Five_Green.png"];
    ...
    dragSprite2 = [CCSprite spriteWithFile:@"One_Pink.png"];
    ...
    dragSprite3 = [CCSprite spriteWithFile:@"Six_Yellow.png"];
...
- (void)ccTouchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (!startExpire) {
        ...
        holdTimer = [NSTimer scheduledTimerWithTimeInterval:4.0 target:self selector:@selector(holdExpireSelector) userInfo:nil repeats:NO];
    }
    ...
    if (gameOver) {
        ...
        touchAnimal = CGRectContainsPoint(animalLevel.boundingBox, locationInNodeSpace);
...
- (void)ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ((touchAgain || touchNext || touchPrev || numberLevel || shapeLevel || colourLevel || animalLevel) && gameOver)
    {
        if (touchNext) {
            level++;
            if (level ==6) {
